...
@IBDesignable class RollingPitTabBar: UITabBar {
    ...
    @IBInspectable public var barTopRadius : CGFloat = 5
    @IBInspectable public var barBottomRadius : CGFloat = 10
    
    @IBInspectable public var circleBackColor : UIColor = UIColor(named: "TabCircleColor")!
    @IBInspectable public var circleRadius : CGFloat = 30
    ...
    private func positionItem(barRect : CGRect, totalCount : Int, idx : Int, item : UITabBarItem, view : UIView){
        ...
        if self.selectedItem == item {
            y = barRect.origin.y - (self.circleRadius / 2) // this is use to shift image&Titlte inside/outside circle
        }
